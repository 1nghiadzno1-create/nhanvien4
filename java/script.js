...
function checkIsWearingMask(video, landmarks) {
    ...
    const foreheadPt = landmarks.positions[27];
    const mouthPt = landmarks.positions[62];

    if (!foreheadPt || !mouthPt) return false;

    const foreheadColor = getAverageColor(maskCtx, foreheadPt.x, foreheadPt.y, 5);
    const mouthColor = getAverageColor(maskCtx, mouthPt.x, mouthPt.y, 5);

    const sumF = foreheadColor.r + foreheadColor.g + foreheadColor.b || 1;
    const rf = foreheadColor.r / sumF;
    const gf = foreheadColor.g / sumF;
    const bf = foreheadColor.b / sumF;

    const sumM = mouthColor.r + mouthColor.g + mouthColor.b || 1;
    const rm = mouthColor.r / sumM;
    const gm = mouthColor.g / sumM;
    const bm = mouthColor.b / sumM;

    // Khoảng cách sắc độ (chỉ so sánh màu sắc, bỏ qua độ sáng tối/bóng râm)
    const chromDist = Math.sqrt(Math.pow(rf - rm, 2) + Math.pow(gf - gm, 2) + Math.pow(bf - bm, 2));

    const mouthBrightness = 0.299 * mouthColor.r + 0.587 * mouthColor.g + 0.114 * mouthColor.b;
    const foreheadBrightness = 0.299 * foreheadColor.r + 0.587 * foreheadColor.g + 0.114 * foreheadColor.b;

    // Độ lệch sắc đỏ của trán (da người luôn có sắc đỏ nổi trội: rf - bf > 0.12)
    const foreheadRedBias = rf - bf;
    // Độ lệch sắc đỏ của vùng miệng
    const mouthRedBias = rm - bm;

    console.log(`[Mask Check] ChromDist: ${chromDist.toFixed(3)}, MouthBrightness: ${mouthBrightness.toFixed(1)}, ForeheadRedBias: ${foreheadRedBias.toFixed(3)}, MouthRedBias: ${mouthRedBias.toFixed(3)}`);

    // 1. Phát hiện khẩu trang xanh y tế (sắc xanh dương nổi trội ở miệng, bm lớn hơn hoặc xấp xỉ rm)
    if (bm > rm - 0.05 && bm > 0.30) {
        console.log("[Mask Detected] Khẩu trang y tế xanh (bm > rm - 0.05)");
        return true;
    }

    // 2. Phát hiện khẩu trang trắng (màu trắng trung tính, không có sắc đỏ ấm như da người)
    // Sắc đỏ vùng miệng của người thật luôn ấm (mouthRedBias > 0.10). Khẩu trang trắng có mouthRedBias rất thấp (< 0.07).
    if (mouthBrightness > 150 && mouthRedBias < 0.08 && chromDist > 0.08) {
        console.log("[Mask Detected] Khẩu trang trắng (mouthBrightness > 150 && mouthRedBias < 0.08)");
        return true;
    }

    // 3. Phát hiện khẩu trang đen / tối màu (vùng miệng cực tối so với trán và không có sắc đỏ ấm)
    if (mouthBrightness < 45 && foreheadBrightness > 75 && mouthRedBias < 0.08) {
        console.log("[Mask Detected] Khẩu trang đen (mouthBrightness < 45 && mouthRedBias < 0.08)");
        return true;
    }

    // 4. Phát hiện khẩu trang xám / màu trung tính khác
    if (mouthRedBias < 0.05 && foreheadRedBias > 0.12 && chromDist > 0.08) {
        console.log("[Mask Detected] Khẩu trang xám/trung tính (mouthRedBias < 0.05)");
        return true;
    }
...
async function finalizeCheckout() {
    ...
    try {
        ...
        if (printWindow) {
            printBillIntoWindow(printWindow, rawTotalNum, discountAmount, finalTotal, campaignMessageToPrint, usedVoucherCode);
        }
...
function printBillIntoWindow(printWindow, rawTotal, discountAmount, finalTotal, campaignMessage, usedVoucherCode) {
    let billHTML = `
    <html>
